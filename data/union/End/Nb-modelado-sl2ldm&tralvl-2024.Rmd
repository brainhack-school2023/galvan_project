...
#  suma = sum(t$sl2ldmExp[i], t$ccbdmlExp[i], t$tralvlExp[i], t$ecplvlExp[i], t$ifoldmExp[i] )
  suma = sum(t$sl2ldmExp[i], t$tralvlExp[i]) # solo los 2 mas altos odds: 2.03; 2.34
...
glm.MLE.fit <- mle2(glm_MLE_support,
                    start=list(b0=1.5235615  , b1=2.7578693  , b2=3.9319430  , b3=4.9609336  , b4=5.8249292  , sigma = 0.0926284  ))
summary(glm.MLE.fit)
plot(profile(glm.MLE.fit))
...
b0 <- as.numeric(unlist(glm.MLE.fit@coef[1])) # coef 1 - Intercept
b1 <- as.numeric(unlist(glm.MLE.fit@coef[2])) # coef 2
b2 <- as.numeric(unlist(glm.MLE.fit@coef[3])) # coef 3
b3 <- as.numeric(unlist(glm.MLE.fit@coef[4])) # coef 4
b4 <- as.numeric(unlist(glm.MLE.fit@coef[5])) # coef 5
...
# estos resultados son de ccbdml & ecplvl
#[1] 28984544737    78658624 29297330587
#[1] 28984544737    78658624 29297330587
#            d1       d2          d3
#d1 1.000000000 368.4853 0.989323742
#d2 0.002713813   1.0000 0.002684839
#d3 1.010791470 372.4618 1.000000000
```
